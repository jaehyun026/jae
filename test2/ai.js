...
// 변경: { board: newBoard, move: bestMove } 형태로 반환. move이 null이면 이동 없음.
export function aiMove(board, aiColor = 'black', depth = 3) {
  const moves = getAllLegalMoves(board, aiColor);

  // no legal moves
  if (moves.length === 0) {
    return { board, move: null }; // caller will decide stalemate/checkmate
  }
  ...
  if (!bestMove) {
    // 안전망: 사실 moves.length > 0 이면 여기 안 와야 함
    return { board, move: null };
  }

  const newBoard = simulateMove(cloneBoard(board), bestMove.from.r, bestMove.from.c, bestMove.to.r, bestMove.to.c);
  return { board: newBoard, move: bestMove };
}
